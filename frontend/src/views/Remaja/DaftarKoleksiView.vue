...
export default {
    ...
    methods: {
        loadBuku(page = 1) {
            ...
            axios
                .get(
                    `https://laravel-library-production.up.railway.app/api/Buku/KoleksiBuku?usia=remaja&page=${page}&per_page=${this.perPage}${kategoriParam}`,
                    {
                        headers: {
            ...
            axios
                ...
                .then((responseKategori) => {
                    const allKategori = responseKategori.data.data.filter((k) =>
                        k.usia.some((u) => u.nama.toLowerCase() === 'remaja'),
                    )

                    axios
                        .get(
                            `https://laravel-library-production.up.railway.app/api/Buku/KoleksiBuku?usia=remaja&per_page=${this.perPage}`,
                            {
                                headers: { Authorization: `Bearer ${this.token}` },
                            },
                        )
                        .then((responseBuku) => {
                            const bukuRemaja = responseBuku.data.data

                            const kategoriWithBooks = allKategori.filter((kategoriItem) => {
                                const kategoriName = kategoriItem.kategori.toLowerCase()
                                return bukuRemaja.some((book) =>
                                    book.kategori.some(
        ...
        searchBooks() {
            ...
            axios
                .get(
                    `https://laravel-library-production.up.railway.app/api/Buku/KoleksiBuku?usia=dewasa&per_page=1000`,
                    {
                        headers: { Authorization: `Bearer ${this.token}` },
                    },
                )
